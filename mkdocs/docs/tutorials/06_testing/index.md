...
!!! tip
    ...
    command handler via `resultSatisfying` and the due date contained within the published event via `satisfying`.
    Using `single(...)` asserts in one step that exactly one event was published and that it matches the expected
    shape — no separate `noMore()` call needed.
...
public void canBeReturnedWithoutLateCharge(@Autowired CommandHandlingTestFixture<ReturnBookCommand> fixture) {
    ...
    fixture
            ...
            .succeeds()
            .allEvents()
            .exactly(new BookReturnedEvent(id, 0.0));
}
```
...
```java hl_lines="6-8 25"
    ...
    public void canBeReturnedWithLateChargeCalculated(@Autowired CommandHandlingTestFixture<ReturnBookCommand> fixture) {
        ...
        fixture
                ...
                .succeeds()
                .allEvents()
                .exactly(new BookReturnedEvent(id, 3.2));
    }
```
