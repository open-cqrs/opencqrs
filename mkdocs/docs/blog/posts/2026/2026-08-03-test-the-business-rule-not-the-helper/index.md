...
1.  This phase typing is the central design choice of the test DSL refactor described in the previous article. The compiler refuses to compile chains that mix phases - you cannot call setup methods after `when`, or assertion methods before choosing `succeeds()` or `fails()`.

The previous entry in our writing told the story of how that DSL came to look the way it does. **[We Tried to Delete `expect`. We Got a New Test DSL Instead.](../2026-07-06-we-tried-to-delete-expect/index.md)** describes the refactor in detail. The short version: we redesigned the test API specifically so the type system would push developers toward business-rule tests and away from accidental helper tests. The fluent chain above is the result of that work.
