...
public class Main {
    ...
    public static void main(String[] args) {
        StateRebuildingHandlerDefinition<Object, EventA> srhd = new StateRebuildingHandlerDefinition<>(
                Object.class, EventA.class, (StateRebuildingHandler.FromObject<Object, EventA>)
                        (state, event) -> state != null ? state : new Object());

        CommandHandlingTestFixture<Command> fixture = CommandHandlingTestFixture.withStateRebuildingHandlerDefinitions(
                        srhd)
        ...
        CommandHandlingTestFixture<Command> anotherFixture =
                CommandHandlingTestFixture.withStateRebuildingHandlerDefinitions(srhd)
                        .using(Object.class, new CommandHandler.ForInstanceAndCommand<Object, Command, String>() {
                            @Override
                            public String handle(
                                    Object state, Command command, CommandEventPublisher<Object> publisher) {
        ...
        fixture.given()
                ...
                .event(e -> e.payload(new EventA("4711")).time(Instant.now()).metaData(Map.of("source", "given")))
                .usingCommandSubject()
                ...
                .resultSatisfying(result ->
                        System.out.println("Result: " + result)) // having gerade ziehen nach Vorlage der Validatoren
                .havingState(new Object()) // having gerade ziehen nach Vorlage der Validatoren
                .stateSatisfying(state ->
                        System.out.println("State: " + state)) // state gerade ziehen nach Vorlage der Validatoren
                .stateExtracting(Object::toString, "expectedString") // state gerade ziehen nach Vorlage der Validatoren
                .then()
                .allEvents()
                .single(e -> e.comparing("UserRegistered")
                        .ofType(String.class)
                        .satisfying(event -> {
                            assert event != null;
                ...
                .single(e -> e.comparing("EmailSent")
                        .ofType(String.class)
                        .satisfying(event -> {
                            assert event != null;
                ...
                .single(e -> e.comparing("AuditLogCreated")
                        .ofType(String.class)
                        .satisfying(event -> {
                            assert event != null;
                ...
                .exactly(e -> e.comparing("UserRegistered"))
                .then()
                .nextEvents()
                .single(e -> e.comparing("Event1")
                        .ofType(String.class)
                        .satisfying(event -> {
                            assert event != null;
                            assert event.toString().contains("user");
                        ...
                        .asserting(assertion -> assertion.payloadType(String.class)))
                .single(e -> e.ofType(String.class).comparing("Event2"))
                .exactly(e -> e // Erwartet exakte Reihenfolge dieser Events // Betrachtet so viele Events wie übergeben
                        .ofType(String.class))
                .any(e -> e // In Abgrenzung zu exactly - Erwartet übergebene e in beliebiger Reihenfolge // Betrachtet
                        // Anzahl übergebener e
                        .ofType(String.class))
                .skip(2);
    }
}
