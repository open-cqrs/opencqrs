...
public class EventHandlingProcessor implements Runnable {
    ...
    final EventTracingContextExecutor contextExtractor;
    ...
    EventHandlingProcessor(
            ...
            EventTracingContextExecutor contextExtractor,
                    ...
                    eventReader.consumeRaw(
                            ...
                            (rawCallback, raw) -> {
                                try {
                                    ...
                                    Runnable processEventInRestoredContext =
                                            () -> rawCallback.upcast((upcastedCallback, upcasted) ->
                                                    upcastedCallback.convert((metadata, event) -> {
                                                        Consumer<EventHandlerDefinition> handleEventInNewSpan = ehd -> {
                                                            var spanInformation =
                                                                    createEventHandlingSpanInformation(raw, ehd);
                                                            spanBuilder.executeRunnableWithNewSpan(
                                                                    spanInformation, () -> {
                                                                        try {
                                                                            switch (ehd.handler()) {
                                                                                case EventHandler.ForObject handler ->
                                                                                    handler.handle(event);
                                                                                case EventHandler.ForObjectAndMetaData
                                                                                        handler ->
                                                        ...
                                                        var convertedEventRelevant =
                                                                switch (eventSequenceResolver) {
                                                                    case EventSequenceResolver.ForRawEvent ignored ->
                                                                        true;
                                                                    case EventSequenceResolver
                                                                                    .ForObjectAndMetaDataAndRawEvent
                                                                            esr ->
                                                                        partitionKeyResolver.resolve(esr.sequenceIdFor(
                                                                                        event, metadata))
                                                                                == partition;
                                                                };
                                            ...
                                            .submit(() -> {
                                                progressTracker.proceed(groupId, partition, () -> {
                                                    if (!skipEvent.getAndSet(false)) {
                                                        ...
                                                        if (rawEventRelevant) {
                                                            contextExtractor.executeInRestoreContextFromEvent(
